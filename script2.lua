...
local function keyCodeFromName(nm)
    if not nm or nm == "" then
        return nil
    end
    local ok, code = pcall(function()
        return Enum.KeyCode[nm]
    end)
    if ok then
        return code
    end
...
-- ============ TOAST SYSTEM (Queue, oben rechts) ============
local toastY = 0
local toastGen = 0
local function toast(msg, col)
    ...
    local f = ni("Frame", {
        ...
        Position = UDim2.new(1, 10, 0, 14 + (slot - 1) * 42),
    ...
    TS:Create(f, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { BackgroundTransparency = 0, Position = UDim2.new(1, -14, 0, 14 + (slot - 1) * 42) }):Play()
    ...
    task.delay(1.4, function()
        pcall(function()
            TS:Create(f, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), { BackgroundTransparency = 1, Position = UDim2.new(1, 10, 0, 14 + (slot - 1) * 42) }):Play()
...
local closeStroke = ni("UIStroke", { Color = RED, Thickness = 1, Transparency = 1, Parent = closeBtn })
closeBtn.MouseEnter:Connect(function()
    TS:Create(closeBtn, TweenInfo.new(0.12), { BackgroundColor3 = BG4 }):Play()
    TS:Create(closeStroke, TweenInfo.new(0.12), { Transparency = 0.4 }):Play()
end)
closeBtn.MouseLeave:Connect(function()
    TS:Create(closeBtn, TweenInfo.new(0.12), { BackgroundColor3 = BG3 }):Play()
    TS:Create(closeStroke, TweenInfo.new(0.12), { Transparency = 1 }):Play()
end)
...
hideBtn.MouseEnter:Connect(function()
    TS:Create(hideBtn, TweenInfo.new(0.12), { BackgroundColor3 = BG4, TextColor3 = TXT }):Play()
end)
hideBtn.MouseLeave:Connect(function()
    TS:Create(hideBtn, TweenInfo.new(0.12), { BackgroundColor3 = BG3, TextColor3 = SUB }):Play()
end)
local showWin, hideWin
hideBtn.MouseButton1Click:Connect(function()
    hideWin()
end)

local miniBadge = nil
local function showMiniBadge(fromPos, fromSize)
    if miniBadge then
        return
    end
    miniBadge = ni("TextButton", {
        Position = fromPos or UDim2.new(0, 10, 0.4, 0),
        Size = fromSize or UDim2.new(0, 40, 0, 40),
        BackgroundColor3 = ACC,
        BorderSizePixel = 0,
        Text = "NH",
    ...
    if fromPos then
        TS:Create(miniBadge, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Position = UDim2.new(0, 10, 0.4, 0),
            Size = UDim2.new(0, 40, 0, 40),
        }):Play()
    end
    miniBadge.MouseButton1Click:Connect(function()
        local bPos = miniBadge.Position
        local bSize = miniBadge.Size
        pcall(function() miniBadge:Destroy() end)
        miniBadge = nil
        win.Visible = true
        win.Position = UDim2.new(0, bPos.X.Offset, 0, bPos.Y.Offset)
        win.Size = bSize
        TS:Create(win, TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Position = UDim2.new(0.5, -math.floor(UI_BASE * S.uiScale + 0.5) / 2, 0.5, -math.floor(UI_BASE * S.uiScale + 0.5) / 2),
            Size = UDim2.new(0, math.floor(UI_BASE * S.uiScale + 0.5), 0, math.floor(UI_BASE * S.uiScale + 0.5)),
        }):Play()
    end)
end
...
miniBtn.MouseEnter:Connect(function()
    TS:Create(miniBtn, TweenInfo.new(0.12), { BackgroundColor3 = BG4 }):Play()
end)
miniBtn.MouseLeave:Connect(function()
    TS:Create(miniBtn, TweenInfo.new(0.12), { BackgroundColor3 = BG3 }):Play()
end)
miniBtn.MouseButton1Click:Connect(function()
    local startPos = win.Position
    local startSize = win.Size
    hideWin(function()
        showMiniBadge(startPos, startSize)
    end)
end)

-- Sidebar + Tabs (Bild-Look + Tab-Glide)
...
local TAB_ICONS = { "🏠", "🎯", "👁", "📦", "🎬", "🏃", "👥", "📍", "⚙" }
local tabBtns, tabLbls, tabIcons, tabSel = {}, {}, {}, {}

local glidePill = ni("Frame", {
...
ni("UICorner", { CornerRadius = UDim.new(0, 8), Parent = glidePill })
ni("UIGradient", {
    Rotation = 115,
    Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 255, 255)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(185, 190, 215)),
    }),
    Parent = glidePill,
})
table.insert(themeAcc, { glidePill, "BackgroundColor3" })

local function paintTab(i)
    local sel = tabSel[i]
    if sel then
        TS:Create(glidePill, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Position = tabBtns[i].Position,
            Size = tabBtns[i].Size,
        }):Play()
    end
    tabLbls[i].TextColor3 = sel and TXT or SUB
    tabIcons[i].TextColor3 = sel and TXT or SUB
end

for i = 1, #TAB_NAMES do
    tabSel[i] = false
    local b = ni("TextButton", {
        Position = UDim2.new(0, 8, 0, 8 + (i - 1) * 34),
        Size = UDim2.new(1, -16, 0, 30),
        BackgroundColor3 = BG2,
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
    ...
    b.MouseEnter:Connect(function()
        if not tabSel[i] then
            TS:Create(glidePill, TweenInfo.new(0.12), { BackgroundColor3 = BG3 }):Play()
        end
    end)
    b.MouseLeave:Connect(function()
        if not tabSel[i] then
            TS:Create(glidePill, TweenInfo.new(0.12), { BackgroundColor3 = ACC }):Play()
        end
    end)
end

-- Status-Karte unten
local statusCard = ni("Frame", {
...
ni("UICorner", { CornerRadius = UDim.new(0, 8), Parent = statusCard })
ni("UIStroke", { Color = BG4, Thickness = 1, Transparency = 0.5, Parent = statusCard })
...
task.spawn(function()
    while statusCard.Parent do
        pcall(function()
            TS:Create(stDot, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { BackgroundTransparency = 0.6 }):Play()
        end)
        task.wait(0.7)
        pcall(function()
            TS:Create(stDot, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { BackgroundTransparency = 0 }):Play()
        end)
        task.wait(0.7)
    end
end)
...
-- Resize-Grip (unten rechts ziehen)
...
track(UIS.InputChanged:Connect(function(inp)
    if resizing and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
        local nw = math.clamp(inp.Position.X - win.AbsolutePosition.X, 320, 720)
        local nh = math.clamp(inp.Position.Y - win.AbsolutePosition.Y, 300, 720)
        S.uiScale = clamp(math.max(nw, nh) / UI_BASE, 0.7, 1.6)
...
track(UIS.InputEnded:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
        if resizing then
            ...
            toast("Größe: " .. tostring(math.floor(UI_BASE * S.uiScale + 0.5)) .. "px", GRN)
        end
    end
end))
ni("UICorner", { CornerRadius = UDim.new(0, 12), Parent = footer })

-- Fenster Open/Close Animation
local winAnim = false
function showWin()
    if win.Visible or winAnim then
        return
    end
    winAnim = true
    win.Visible = true
    win.BackgroundTransparency = 0.4
    local baseY = UDim2.new(0.5, 0, 0.5, 0)
    win.Position = UDim2.new(0.5, -win.AbsoluteSize.X / 2, 0.5, -win.AbsoluteSize.Y / 2 + 14)
    TS:Create(win, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
function hideWin(cb)
    winAnim = true
    TS:Create(win, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
        BackgroundTransparency = 0.4,
    }):Play()
    TS:Create(win, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
        Position = UDim2.new(win.Position.X.Scale, win.Position.X.Offset, win.Position.Y.Scale, win.Position.Y.Offset + 14),
    }):Play()
...
local function addToggle(page, titleTxt, key, doToast, onSet)
    ...
    table.insert(themeAcc, { accent, "BackgroundColor3" })
    ...
    local function setOn(v, silent)
        ...
        val = v
        S[key] = v
        local kPos = v and UDim2.new(1, -9, 0.5, 0) or UDim2.new(0, 9, 0.5, 0)
        local kCol = v and TXT or SUB
        local accT = v and 0 or 1
        if inited and changed then
            TS:Create(track, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundColor3 = v and ACC or BG4 }):Play()
            TS:Create(knob, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Position = kPos, BackgroundColor3 = kCol }):Play()
            TS:Create(accent, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundTransparency = accT }):Play()
        else
            track.BackgroundColor3 = v and ACC or BG4
            knob.Position = kPos
            knob.BackgroundColor3 = kCol
            accent.BackgroundTransparency = accT
        end
        if onSet then
...
local function addKeybind(page, titleTxt, key)
    local row = mkRow(page)
    ni("TextLabel", {
        Position = UDim2.new(0, 10, 0, 0),
        Size = UDim2.new(1, -100, 1, 0),
    ...
    table.insert(themeAcc, { val, "TextColor3" })
    row.MouseButton1Click:Connect(function()
        val.Text = "Taste drücken..."
        val.TextColor3 = CYAN
        listenBind = { key = key, lbl = val }
    end)
    CTRLS[key] = { set = function(v)
        S[key] = tostring(v or "")
        val.Text = (S[key] ~= "" and S[key] or "keine")
    end }
end
...
addKeybind(setPage, "Custom AimLock-Taste", "cAimKey")
addKeybind(setPage, "Custom GUI-Taste", "cGuiKey")
...
local function buildHomeTab()
    ...
    task.spawn(function()
        ...
        while homePage and homePage.Parent do
            ...
            pcall(function()
                local t = getKeyTimeLeft()
                keyTimeLbl.Text = "Key läuft ab in: " .. t
                keyTimeLbl.TextColor3 = (t == "Abgelaufen" or t == "Kein Key") and RED or GRN
            end)
        end
...
local function getKeyData()
    ...
    local k, ts = raw:match("^(.-)|(%d+)$")
    if not k or not ts then
        return nil, nil
    end
    return k, tonumber(ts)
end

local keySaved = false
do
    local k, ts = getKeyData()
    if k and ts and keyIsValid(k) and (os.time() - ts) < KEY_DURATION then
        keySaved = true
    end
end

getKeyTimeLeft = function()
    local k, ts = getKeyData()
    if not k or not ts or not keyIsValid(k) then
        return "Kein Key"
    end
    local left = KEY_DURATION - (os.time() - ts)
    if left <= 0 then
        return "Abgelaufen"
    end
    return string.format("%dh %02dm %02ds", math.floor(left / 3600), math.floor((left % 3600) / 60), left % 60)
end

local keyHdrLbl = ni("TextLabel", {
...
task.spawn(function()
    while keyHdrLbl.Parent do
        local t = getKeyTimeLeft()
        keyHdrLbl.Text = "⏳ " .. t
        keyHdrLbl.TextColor3 = (t == "Abgelaufen" or t == "Kein Key") and RED or GRN
        task.wait(1)
    end
end)
...
if keySaved then
    ...
    ni("UICorner", { CornerRadius = UDim.new(0, 12), Parent = kWin })
    local kStroke = ni("UIStroke", { Color = ACC, Thickness = 1.2, Transparency = 0.5, Parent = kWin })
    table.insert(themeAcc, { kStroke, "Color" })

    local kSide = ni("Frame", {
    ...
    ni("UICorner", { CornerRadius = UDim.new(0, 12), Parent = kSide })
    ni("TextLabel", {
    ...
    local kNav = { "🏠  Home", "⚡  Executor", "⚙️  Settings", "ℹ️  Info" }
    for i, n in ipairs(kNav) do
        local navBtn = ni("TextLabel", {
            Position = UDim2.new(0, 12, 0, 80 + (i - 1) * 36),
            Size = UDim2.new(1, -24, 0, 30),
            BackgroundColor3 = (i == 1) and ACC or BG2,
            BackgroundTransparency = (i == 1) and 0 or 1,
            BorderSizePixel = 0,
            Text = n,
            TextColor3 = (i == 1) and TXT or SUB,
            Font = Enum.Font.GothamMedium,
            TextSize = 13,
            TextXAlignment = Enum.TextXAlignment.Left,
            ZIndex = 93,
        ...
        ni("UICorner", { CornerRadius = UDim.new(0, 8), Parent = navBtn })
    end

    local kStatus = ni("Frame", {
        AnchorPoint = Vector2.new(0, 1),
    ...
    ni("UICorner", { CornerRadius = UDim.new(0, 8), Parent = kStatus })
    local kDot = ni("Frame", {
    ...
    ni("UICorner", { CornerRadius = UDim.new(1, 0), Parent = kDot })
    ni("TextLabel", {
    ...
    local kBanner = ni("Frame", {
        Position = UDim2.new(0, 182, 0, 16),
        Size = UDim2.new(1, -198, 0, 120),
        BackgroundColor3 = BG2,
        BorderSizePixel = 0,
        ZIndex = 92,
        Parent = kWin,
    })
    ni("UICorner", { CornerRadius = UDim.new(0, 10), Parent = kBanner })
    ni("UIGradient", {
        Rotation = 15,
        Color = ColorSequence.new({
    ...
    ni("UICorner", { CornerRadius = UDim.new(1, 0), Parent = kShield })
    table.insert(themeAcc, { kShield, "TextColor3" })
    ni("TextLabel", {
    ...
    ni("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = "Enter your key to unlock NanoHub.\nYour access will be remembered.",
        TextColor3 = SUB,
        Font = Enum.Font.Gotham,
        TextSize = 13,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    local kCard = ni("Frame", {
        Position = UDim2.new(0, 182, 0, 150),
        Size = UDim2.new(1, -198, 0, 194),
        BackgroundColor3 = BG2,
        BorderSizePixel = 0,
        ZIndex = 92,
        Parent = kWin,
    })
    ni("UICorner", { CornerRadius = UDim.new(0, 10), Parent = kCard })
    ni("TextLabel", {
    ...
    ni("UICorner", { CornerRadius = UDim.new(0, 8), Parent = kBox })
    ni("UIStroke", { Color = BG4, Thickness = 1, Transparency = 0.4, Parent = kBox })
    local kMsg = ni("TextLabel", {
    ...
    ni("UICorner", { CornerRadius = UDim.new(0, 8), Parent = kBtn })
    table.insert(themeAcc, { kBtn, "BackgroundColor3" })
    ni("TextLabel", {
    ...
    local function verify()
        local k = kBox.Text
        if keyIsValid(k) then
            kMsg.TextColor3 = GRN
            kMsg.Text = "Key verified! Loading..."
            if canFS then
                pcall(function()
                    if makefolder and isfolder and not isfolder(CFG_DIR) then
                        makefolder(CFG_DIR)
                    end
                    writefile(KEY_FILE, VALID_KEY .. "|" .. tostring(os.time()))
                end)
            end
            task.wait(0.5)
            win.Visible = true
            ...
            kMsg.Text = "Invalid key! Try again."
            kBox.Text = ""
            TS:Create(kBox, TweenInfo.new(0.08), { Position = UDim2.new(0, 22, 0, 48) }):Play()
            task.delay(0.08, function()
                TS:Create(kBox, TweenInfo.new(0.08), { Position = UDim2.new(0, 10, 0, 48) }):Play()
            end)
        end
    end
...
track(UIS.InputBegan:Connect(function(inp, g)
    ...
    if listenBind then
        if inp.UserInputType == Enum.UserInputType.Keyboard then
            if inp.KeyCode == Enum.KeyCode.Escape then
                S[listenBind.key] = ""
                listenBind.lbl.Text = "keine"
            else
                S[listenBind.key] = inp.KeyCode.Name
                listenBind.lbl.Text = inp.KeyCode.Name
            end
            listenBind.lbl.TextColor3 = ACC
            listenBind = nil
            autoSave()
        end
        return
    ...
    if inp.KeyCode == GUIKEY_CODES[S.guiKeyIdx] or keyCodeFromName(S.cGuiKey) == inp.KeyCode then
    ...
    elseif S.keyEnabled and (inp.KeyCode == curKey() or keyCodeFromName(S.cAimKey) == inp.KeyCode) then
        if aimSet then
            aimSet.set(not S.on)
        end
    else
...
-- ============ ESP + CHAMS LOOP (Frame-basiert, kein Drawing) ============
local espGui = ni("ScreenGui", {
    Name = rndName("ESPG"),
    ResetOnSpawn = false,
    IgnoreGuiInset = true,
    DisplayOrder = 40,
...
local function espClearOne(pl)
    ...
    pcall(function()
        ...
        if e.name then e.name:Destroy() end
        ...
        if e.hpBg then e.hpBg:Destroy() end
        if e.hpF then e.hpF:Destroy() end
...
track(RS.RenderStepped:Connect(function()
    ...
    for _, pl in ipairs(P:GetPlayers()) do
        if pl ~= LP then
            ...
            local hrp = c and (c:FindFirstChild("HumanoidRootPart") or c:FindFirstChild("UpperTorso") or c:FindFirstChild("Torso"))

            -- Chams (Highlight, kein Drawing)
            local chamsOk = false
            if S.chamsOn and c and hum and hum.Health > 0 then
                if not (S.teamCheck and pl.Team ~= nil and pl.Team == LP.Team) then
                    pcall(function()
            ...
            if not chamsOk and e.hl then
                pcall(function() e.hl:Destroy() end)
                e.hl = nil
            end

            -- ESP (Frames)
            if S.espOn and canDraw ~= nil and c and head and hrp and hum and hum.Health > 0 then
                if not (S.teamCheck and pl.Team ~= nil and pl.Team == LP.Team) then
                    ...
                    if dist <= S.espMaxD then
                        ...
                        if spT.Z > 0 and spB.Z > 0 then
                            ...
                            local yTop = spT.Y
                            local yBot = spB.Y
                            local bh = math.max(yBot - yTop, 10)
                            local bw = bh * 0.46
                            local cx = spT.X

                            -- Tracer (rotiertes Frame von Bildschirm-Unterkante zur Box)
                            if S.espTracer then
                                if not e.tracer then
                                    ...
                                    table.insert(themeAcc, { e.tracer, "BackgroundColor3" })
                                end
                                local tx1, ty1 = vp.X / 2, vp.Y
                                local tx2, ty2 = cx, yBot
                                local dx, dy = tx2 - tx1, ty2 - ty1
                                local len = math.sqrt(dx * dx + dy * dy)
                                e.tracer.Visible = true
                                e.tracer.BackgroundColor3 = ec
                                e.tracer.AnchorPoint = Vector2.new(0.5, 0.5)
                                e.tracer.Position = UDim2.new(0, (tx1 + tx2) / 2, 0, (ty1 + ty2) / 2)
                                e.tracer.Size = UDim2.new(0, len, 0, 1)
                                e.tracer.Rotation = math.deg(math.atan2(dy, dx))
                            elseif e.tracer then
                                e.tracer.Visible = false
                            end

                            -- Skeleton (Linien über Knochen, rotierte Frames)
                            if S.espSkel then
                                if not e.skel then
                                    e.skel = {}
                                end
                                ...
                                for bi = 1, 15 do
                                    ...
                                    if not ln then
                                        ...
                                        table.insert(themeAcc, { ln, "BackgroundColor3" })
                                        e.skel[bi] = ln
                                    end
                                    if bone then
                                        local pa = c:FindFirstChild(bone[1])
                                        local pb = c:FindFirstChild(bone[2])
                                        if pa and pb and pa:IsA("BasePart") and pb:IsA("BasePart") then
                                            local p1 = v:WorldToViewportPoint(pa.Position)
                                            local p2 = v:WorldToViewportPoint(pb.Position)
                                            if p1.Z > 0 and p2.Z > 0 then
                                                local ddx, ddy = p2.X - p1.X, p2.Y - p1.Y
                                                local llen = math.sqrt(ddx * ddx + ddy * ddy)
                                                ln.Visible = true
                                                ln.BackgroundColor3 = ec
                                                ln.AnchorPoint = Vector2.new(0.5, 0.5)
                                                ln.Position = UDim2.new(0, (p1.X + p2.X) / 2, 0, (p1.Y + p2.Y) / 2)
                                                ln.Size = UDim2.new(0, llen, 0, 1)
                            ...
                            if not e.box then
                                ...
                                local bs = ni("UIStroke", { Color = ec, Thickness = 1.5, Parent = e.box })
                                e.boxStroke = bs
                                table.insert(themeAcc, { bs, "Color" })
                            end
                            e.box.Visible = true
                            e.box.Position = UDim2.new(0, cx - bw / 2, 0, yTop)
                            e.box.Size = UDim2.new(0, bw, 0, bh)
                            e.boxStroke.Color = ec
                            ...
                            if S.espHP then
                                if not e.hpBg then
                                    ...
                                    ni("UICorner", { CornerRadius = UDim.new(1, 0), Parent = e.hpBg })
                                    e.hpF = ni("Frame", {
                                        BackgroundColor3 = GRN,
                                        BorderSizePixel = 0,
                                        ZIndex = 22,
                                    ...
                                    ni("UICorner", { CornerRadius = UDim.new(1, 0), Parent = e.hpF })
                                end
                                ...
                                e.hpBg.Visible = true
                                e.hpBg.Position = UDim2.new(0, barX, 0, yTop)
                                e.hpBg.Size = UDim2.new(0, 4, 0, bh)
                                e.hpF.Visible = true
                                e.hpF.BackgroundColor3 = RED:Lerp(GRN, ratio)
                                e.hpF.Position = UDim2.new(0, 0, 1 - ratio, 0)
                                e.hpF.Size = UDim2.new(1, 0, ratio, 0)
                            elseif e.hpBg then
                            ...
                            if S.espName then
                                ...
                                e.name.Visible = true
                                e.name.TextColor3 = ec
                                e.name.Text = pl.Name .. " (" .. string.format("%.0f", dist) .. "m)"
                                e.name.Position = UDim2.new(0, cx - 80, 0, yTop - 20)
                                e.name.Size = UDim2.new(0, 160, 0, 16)
                            elseif e.name then
